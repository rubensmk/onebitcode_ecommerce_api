...
RSpec.describe License, type: :model do
  ...
  it_behaves_like "with paginatable concern", :license
  it_has_behavior_of "with like searchable concern", :license, :key

  it "must have a :line_item if it's :in_use" do
    subject.status = "in_use"
    expect(subject).to validate_presence_of(:line_item)
  end
end
