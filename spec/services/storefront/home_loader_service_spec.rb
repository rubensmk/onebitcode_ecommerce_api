...
describe Storefront::HomeLoaderService do
  context "when #call" do
    ...
    context "with on featured products" do
      ...
      it "does not return unavailable or non-featured products" do
        service = described_class.new
        service.call
        expect(service.featured).not_to include(unavailable_products, non_featured_products)
      end
    end

    context "with on recently released procucts" do
      ...
      it "does not return non-last released or unavailable products" do
        service = described_class.new
        service.call
        expect(service.last_releases).not_to include(unavailable_products, non_last_release_products)
      end
    end

    context "with on cheapest products" do
      ...
      it "returns non-cheapest or unavailable products" do
        service = described_class.new
        service.call
        expect(service.cheapest).not_to include(unavailable_products, non_cheapest)
      end
    end
  end
end
